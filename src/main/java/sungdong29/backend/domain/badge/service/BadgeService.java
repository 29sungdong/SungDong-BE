...
public class BadgeService {
    ...
    public BadgesResponseDTO findUserBadges(Long userId) {
        ...
        Map<Badge, List<UserBadgeResponseDTO>> badgeUserBadgeMap = userBadges.stream()
                .collect(Collectors.groupingBy(UserBadge::getBadge,
                        Collectors.mapping(UserBadgeResponseDTO::from, Collectors.toList())));

        List<BadgeResponseDTO> badgeResponseDTOS = badges.stream()
                .map(badge -> BadgeResponseDTO.of(badge, badgeUserBadgeMap.getOrDefault(badge, List.of())))
